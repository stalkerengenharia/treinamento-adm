...
class Cache{
	...
	/**
	 * Total de horas
	 * para expirar o cache.
	 * 
	 * @access private
	 * @name $time
	 * @var time
	 */
	private static $time = '100000 hours';
	...
	public function __construct($folder = null) {
		...
		$this->setFolder(!is_null($folder) ? $this->folder."/".$folder : "data/cache");
	...
	public function clean($key=null){
		...
		if (is_dir($this->folder)) { 
	        ...
	        $objects = scandir($this->folder); 
	        
	        foreach ($objects as $object) { 
		        if ($object != "." && $object != "..") { 
		            if (filetype($this->folder."/".$object) == "dir"){
		                 //deleteDirectory($this->folder."/".$object);
		            }else{
		                 unlink($this->folder."/".$object);
		            } 
		        } 
