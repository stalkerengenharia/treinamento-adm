<form action='<?php echo stalker\library\Fw\Vital::getUrl("Form","salvar"); ?>' method='post' data-ajax="false">
	...
	<div class="ui-content">
		...
		<input type="email" class='required email' name="email" id="email" value="<?php echo $this->view->data['email']; ?>" />

		<input type='hidden' name='nome' value='<?php echo $this->view->data['nome']; ?>' />
		
		<label for="nome"><strong><em>*</em> Nome</strong>:</label>
		<input type="text" class='required name' name="nome" id="nome" value="<?php echo $this->view->data['nome']; ?>" />
		
		<input type='hidden' name='senha' value='<?php echo $this->view->data['senha']; ?>' />
		
		<label for="senha"><strong><em>*</em> Senha</strong>:</label>
		<input type="password" class='required' name="senha" id="senha" value="<?php echo $this->view->data['senha']; ?>" />
